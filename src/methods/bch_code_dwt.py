...
def encode_bch_dwt(orig_video_path, message_path, xor_key, col_key, row_key, bch_num=11):
    ...
    positions = vid_utils.get_positions(
        col_key, row_key, width//2, height//2)  # width, height)
    used_positions = set()
    ...
    for i in range(0, len(chaos_arr), bch_num):
        ...
        LH_Y[row, col] = int(LH_Y_bin[:-3] + ''.join(str(bit)
                             for bit in codeword[:3]), 2)
        HL_Y[row, col] = int(HL_Y_bin[:-3] + ''.join(str(bit)
                             for bit in codeword[3:6]), 2)
        HH_Y[row, col] = int(HH_Y_bin[:-3] + ''.join(str(bit)
                             for bit in codeword[6:9]), 2)
    ...
    vid_utils.reconstruct_video_from_rgb_frames(
        orig_video_path, vid_properties)
...
def process_binary_array(binary_array, encode=True):
    ...
    if encode:
        if original_length % 8 != 0:
            ...
            binary_array = np.append(binary_array, np.zeros(
                padding_length, dtype=np.uint8))
    else:
    ...
    for float_pair in FLOAT_PAIRS:
        ...
        generatedKey = logistic_key(
            segment_length, float_pair[0], float_pair[1])
        process_part_component(binary_array, chaos_array,
                               start_index, end_index, create_B(generatedKey))
        i += 1
